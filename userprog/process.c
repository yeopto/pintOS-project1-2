...
process_exec (void *f_name) {  // 유저가 입력한 명령어를 수행하도록 프로그램을 메모리에 적재하고 실행하는 함수. 여기에 파일 네임 인자로 받아서 저장(문자열) -> 근데 실행 프로그램 파일과 옵션이 분리되지 않은 상황
	char *file_name = f_name; // 위에서 인자로 받은 문자열은 void 형인데 우리가 받은 명령어는 문자열이니 문자열 포인터로 변환
	bool success;
	char *token, *save_ptr;
	int argc=0;
	char *argv[30];
	...
	for (token = strtok_r (file_name, " ", &save_ptr); token != NULL; // // 첫번째 이름을 받아온다. save_ptr: 앞에 애 자르고 남은 문자열의 가장 맨 앞을 가리키는 포인터 주소 값 
	...
	struct intr_frame _if; // intr_frame 내 구조체 멤버에 필요한 정보를 담는다. 여기서 intr_frame은 인터럽트 스택 프레임. 기존까지 실행 중이던 context를 스택에 저장하기 위한 구조체
	_if.ds = _if.es = _if.ss = SEL_UDSEG;
	_if.cs = SEL_UCSEG;
	_if.eflags = FLAG_IF | FLAG_MBS;

	/* We first kill the current context */
	process_cleanup (); // 새로운 실행 파일을 현재 스레드에 담기 전에 먼저 현재 프로세스에 담긴 context를 지워줌 -> 현재 프로세스에 할당된 page directory를 지운다는 뜻

	/* And then load the binary */
	success = load (file_name, &_if); // file_name, _if를 현재 프로세스에 적재 함(초기화) -> success는 bool type이라서 성공하면 1, 실패하면 0을 반환

	/* If load failed, quit. */
	if (!success)
	{
		palloc_free_page(file_name); // load
		return -1;
	}
	...
	hex_dump(_if.rsp, _if.rsp, USER_STACK - _if.rsp, true); // 디버깅용 툴

	/* Start switched process. */
	do_iret (&_if); // 만약 load가 실행됐다면 context switching을 진행함.
	NOT_REACHED ();
}
...
static void argument_stack(struct intr_frame *if_, int argv_cnt, char **argv_list) {
	...
	for (i = argv_cnt-1; i >= 0; i--){
		...
		각 인자에서 인자 크기를 읽고 
		그 크기만큼 rsp를 내려주고. 그 다음 빈 공간만큼 memcpy해줌
		*/
...
process_wait (tid_t child_tid UNUSED) {
	...
	while(1){} // 자식 프로세스 종료될때 까지 무한히 기다리기
	return -1;
}
